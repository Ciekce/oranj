...
#define OJ_SIMD_ALIGNAS alignas(oranj::util::simd::kAlignment)
...
#define OJ_SIMD_OP_0(Name) \
    ...
    template <> \
    OJ_ALWAYS_INLINE_NDEBUG inline auto Name<i8>() { \
        return impl::Name##I8(); \
    } \
    template <> \
...
#define OJ_SIMD_OP_1_VALUE(Name, Arg0) \
    ...
    template <> \
    OJ_ALWAYS_INLINE_NDEBUG inline auto Name<i8>(i8 Arg0) { \
...
#define OJ_SIMD_OP_2_VECTORS(Name, Arg0, Arg1) \
    ...
    template <> \
    OJ_ALWAYS_INLINE_NDEBUG inline auto Name<i8>(Vector<i8> Arg0, Vector<i8> Arg1) { \
...
#define OJ_SIMD_OP_3_VECTORS(Name, Arg0, Arg1, Arg2) \
    ...
    template <> \
    OJ_ALWAYS_INLINE_NDEBUG inline auto Name<i8>(Vector<i8> Arg0, Vector<i8> Arg1, Vector<i8> Arg2) { \
    ...
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto load<u8>(const void* ptr) {
        return impl::loadU8(ptr);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto load<i8>(const void* ptr) {
    ...
    OJ_ALWAYS_INLINE_NDEBUG inline Vector<i16> widenLoadI8ToI16(const void* ptr) {
    ...
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto store<u8>(void* ptr, Vector<u8> v) {
        impl::storeU8(ptr, v);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto store<i8>(void* ptr, Vector<i8> v) {
        impl::storeI8(ptr, v);
    }
    template <>
    ...
    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeft(Vector<T> v, i32 shift) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeft<i8>(Vector<i8> v, i32 shift) {
        return impl::shiftLeftI8(v, shift);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeft<i16>(Vector<i16> v, i32 shift) {
        return impl::shiftLeftI16(v, shift);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeft<i32>(Vector<i32> v, i32 shift) {
        return impl::shiftLeftI32(v, shift);
    }

    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftRight(Vector<T> v, i32 shift) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftRight<i16>(Vector<i16> v, i32 shift) {
        return impl::shiftRightI16(v, shift);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftRight<i32>(Vector<i32> v, i32 shift) {
        return impl::shiftRightI32(v, shift);
    }

    template <typename T, i32 kShift>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shift(Vector<T> v) {
    ...
    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto mulLo(Vector<T> a, Vector<T> b) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto mulLo<i16>(Vector<i16> a, Vector<i16> b) {
        return impl::mulLoI16(a, b);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto mulLo<i32>(Vector<i32> a, Vector<i32> b) {
        return impl::mulLoI32(a, b);
    }

    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeftMulHi(Vector<T> a, Vector<T> b, i32 shift) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto shiftLeftMulHi<i16>(Vector<i16> a, Vector<i16> b, i32 shift) {
        return impl::shiftLeftMulHiI16(a, b, shift);
    }

    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto packUnsigned(Vector<T> a, Vector<T> b) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto packUnsigned<i16>(Vector<i16> a, Vector<i16> b) {
        return impl::packUnsignedI16(a, b);
    }
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto packUnsigned<i32>(Vector<i32> a, Vector<i32> b) {
    ...
    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto dpbusd(Vector<T> sum, Vector<u8> u, Vector<i8> i) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto dpbusd<i32>(Vector<i32> sum, Vector<u8> u, Vector<i8> i) {
        return impl::dpbusdI32(sum, u, i);
    }

    template <typename T>
    OJ_ALWAYS_INLINE_NDEBUG inline auto nonzeroMask(Vector<T> v) = delete;
    template <>
    OJ_ALWAYS_INLINE_NDEBUG inline auto nonzeroMask<u8>(Vector<u8> v) {
