...
#define OJ_SPARSE_BENCH_FT_SIZE 0

namespace oranj::eval::nnue::arch::sparse {
#if OJ_SPARSE_BENCH_FT_SIZE > 0
    inline std::array<usize, OJ_SPARSE_BENCH_FT_SIZE / 2> g_activationCounts{};
    inline void trackActivations(std::span<const u8, OJ_SPARSE_BENCH_FT_SIZE> ftActivations) {
        for (usize i = 0; i < OJ_SPARSE_BENCH_FT_SIZE; ++i) {
            if (ftActivations[i] != 0) {
                ++g_activationCounts[i % (OJ_SPARSE_BENCH_FT_SIZE / 2)];
            }
        }
    }
#endif
    ...
    private:
        ...
        OJ_ALWAYS_INLINE_NDEBUG static Vector128U16 zero() {
            return vdupq_n_u16(0);
        }

        OJ_ALWAYS_INLINE_NDEBUG static Vector128U16 set1(u16 v) {
            return vdupq_n_u16(v);
        }

        OJ_ALWAYS_INLINE_NDEBUG static Vector128U16 load(const void* ptr) {
            assert(util::isAligned<kAlignment>(ptr));
            return vld1q_u16(reinterpret_cast<const u16*>(ptr));
        }

        OJ_ALWAYS_INLINE_NDEBUG static void ustore(void* ptr, Vector128U16 v) {
            return vst1q_u16(reinterpret_cast<u16*>(ptr), v);
        }

        OJ_ALWAYS_INLINE_NDEBUG static Vector128U16 add(Vector128U16 a, Vector128U16 b) {
